...
def export_to_sarif(
    findings: List[Dict[str, Any]], tool_name: str = "security-scanner", tool_version: str = "1.0.0"
) -> str:
    """
    ...
    for tool, tool_findings in findings_by_tool.items():
        ...
        run = {
            "tool": {
                "driver": {"name": tool, "version": tool_version, "informationUri": f"https://example.com/tools/{tool}"}
            },
            "results": results,
        }
    ...
    sarif = {"version": SARIF_VERSION, "$schema": SARIF_SCHEMA, "runs": runs}
...
def export_to_pdf(
    findings: List[Dict[str, Any]], scan_info: Dict[str, Any] = None, analytics_data: Dict[str, Any] = None
) -> bytes:
    """
